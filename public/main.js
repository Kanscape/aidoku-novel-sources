document.addEventListener("DOMContentLoaded", () => {
	...
	const addRepositoryButton = document.getElementById(
		"add-repository-button",
	);
	const sourceListUrlNode = document.getElementById("source-list-url");
	const sourceListUrl = new URL("index.min.json", window.location.href).href;

	if (addRepositoryButton) {
		addRepositoryButton.addEventListener("click", () => {
			window.location =
				"aidoku://addSourceList?url=" +
				encodeURIComponent(sourceListUrl);
		});
	}
	if (sourceListUrlNode) {
		sourceListUrlNode.textContent = sourceListUrl;
	}
